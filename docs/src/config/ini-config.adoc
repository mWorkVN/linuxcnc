...
#<_ini[section]name>. See <<gcode:parameters,G-code Parameters>>.
...
See <<gcode:parameters,G-code Parameters>> for more details and an
important caveat.
...
* HOMEMOD = homing_module (Optional)  If specified,
use the specified (user-built) module instead of the default homemod.  The
setting may be overridden from the command line using the -m option ($linuxcnc -h)
...
* TPMOD = alternate_trajectory_planning module (Optional)  If specified,
use the specified (user-built) module instead of the default tpmod.  The
...
* 'TOOL_CHANGE_AT_G30 = 1' -
    ...
    <<gcode:parameters,G-Code Parameters>> and
    <<gcode:g30-g30.1,G-Code G30-G30.1>>.
