...
file during the startup of an LinuxCNC. In the example
below, which uses G55, the position of each axis for G55 origin
...
etc are better ways to set the variables. For our example, we will directly
edit the file so that G55 will take the following values:
