...
* GStat checks LinuxCNC's status every 100&thinsp;ms and if there are differences from
...
the joint actual position. Before homing, on multi-joint axes, only joint position is valid.
