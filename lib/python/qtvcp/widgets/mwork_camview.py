...
class mwork_camview(QtWidgets.QWidget, _HalWidgetBase):
    def __init__(self, parent=None, halname=None):
        super(mwork_camview, self).__init__(parent)
    ...
    def wheelEvent(self, event):
        super(mwork_camview, self).wheelEvent(event)
    ...
    def nextFrameSlot(self, w):
        ...
        frame = self.fromRedis(self.r,'image')
        #if not ret: return
        ...
        # repaint the window
        #self.lblImage.setPixmap(QtGui.QPixmap.fromImage(self.pix))
        self.update()
        try:
...
class WebcamVideoStream:
    ...
    def _update(self):
        ...
        if self.stream is None:
            ...
            print('CAMERA OPEN OK', self.camnum)
        while True:
            # if the thread indicator variable is set, stop the thread
            if self.stopped:
                print('CAMERA STOP', self.camnum)
                self.stream.release()
                return
            else:
