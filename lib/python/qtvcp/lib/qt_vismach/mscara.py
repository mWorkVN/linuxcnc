...
d1 =  hal.get_value('scarakins.D1')
d2 =  hal.get_value('scarakins.D2') #350.0
d3 =  hal.get_value('scarakins.D3') 
d4 =  hal.get_value('scarakins.D4')#350.0
d5 =  hal.get_value('scarakins.D5')# 200.0
d6 =  hal.get_value('scarakins.D6')# 0.0
j3min =  0.0
j3max = 50.0
...
table_height = d1+d3-d5-j3max
work = Capture()
table = Collection([
	work,
	Box(-0.35*reach,-0.5*reach, -0.01*d1, 0.35*reach, 0.5*reach, 0.0)])
...
table = Translate([table],0.5*reach,0.0,table_height-100)
...
class Window(QWidget):
    ...
    def __init__(self,sizezoom):
        ...
        size = max(d1+d3+l3_len,d2+d4+d6)
        world = Capture()
        v.setXRotation(2880) # (360 * 16) /2 la 180 do
        v.model = Collection([model, world])
        v.distance = size * sizezoom
        v.near = size * 0.01
        v.far = size * 10.0
        v.tool2view = tooltip
        v.world2view = world
        ...
        #v.setZRotation(108 )
    def setviewX(self,angle):
        self.glWidget.setXRotation(angle) # (360 * 16) /2 la 180 do
    def setviewY(self,angle):
        self.glWidget.setYRotation(angle) # (360 * 16) /2 la 180 do
