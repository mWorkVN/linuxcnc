...
static void home_do_moving_checks(emcmot_joint_t * joint,int jno)
{
    ...
    if (joint->on_pos_limit || joint->on_neg_limit) {
        ...
        if (!(H[jno].home_flags & HOME_IGNORE_LIMITS)) {
            ...
            rtapi_print_msg(RTAPI_MSG_ERR, _("hit limit in home state %d"), H[jno].home_state);
    ...
    if (!joint->free_tp.active) {
        ...
        rtapi_print_msg(RTAPI_MSG_ERR,_("end of move in home state %d"), H[jno].home_state);
        H[jno].home_state = HOME_ABORT;
        immediate_state = 1;
        return;
    }
...
void homing_init(void)
{
    int i;
#ifdef HOMING_DEBUG
    rtapi_print("HOMING<%s>\n",__FUNCTION__);
    rtapi_print("HOMING                emcmotConfig=%p\n",emcmotConfig);
    rtapi_print("HOMING      vector_SetRotaryUnlock=%p\n",vector_SetRotaryUnlock);
    rtapi_print("HOMING  vector_GetRotaryIsUnlocked=%p\n",vector_GetRotaryIsUnlocked);
    rtapi_print("HOMING                      joints=%p\n",joints);
#endif
    homing_active = 0;
...
{
//Note: id is received from motion
    int jno,retval;
...
bool do_homing(void)
{
    double servo_freq = 1/emcmotConfig->servoCycleTime;
    int allhomed = 0;
    int joint_num;
    emcmot_joint_t *joint;
    double offset, tmp;
    ...
    for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
        ...
        do {
            ...
            case HOME_UNLOCK:
                // unlock now
                vector_SetRotaryUnlock(joint_num, 1);
                H[joint_num].home_state = HOME_UNLOCK_WAIT;
                break;
            ...
            case HOME_UNLOCK_WAIT:
                ...
                if ((H[joint_num].home_flags & HOME_UNLOCK_FIRST) &&
                    !vector_GetRotaryIsUnlocked(joint_num)) break;
            ...
            case HOME_LOCK:
                if (H[joint_num].home_flags & HOME_UNLOCK_FIRST) {
                    vector_SetRotaryUnlock(joint_num, 0);
                } else {
                    immediate_state = 1;
                }
...
{
//Note: This rtapi_app_main() creates a module allow use with
//        'halcmd loadrt homemod'
//      The pins created with export_joint_home_pins() belong to motmod.
//      Additional hal pins could be created herein but would only confuse.
    char* emsg = "?";

    comp_id = hal_init("homemod");  //name must agree with tpmod.so
                     ...
                     )
{
    ...
    emcmotConfig = pconfig;  // ->servoCycleTime
    joints       = pjoints;
}
