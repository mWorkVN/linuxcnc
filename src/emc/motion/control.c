...
/* 'hmod_do_homing_sequence()' decides what, if anything, needs to be done
    related to multi-joint homing.

   no prototype here, implemented in homing.c, proto in mot_priv.h
*/

/* 'hmod_do_homing()' looks at the home_state field of each joint struct
...
void emcmotController(void *arg, long period)
{
    ...
    *(emcmot_hal_data->last_period_ns) = this_run * 1e6 / cpu_khz;
    ...
    hmod_read_homing_in_pins(ALL_JOINTS);
    ...
    output_to_hal();
    hmod_write_homing_out_pins(ALL_JOINTS);
    update_status();
...
static void process_inputs(void)
{
    ...
    if ( enables & AF_ENABLED ) {
        ...
        if ( adaptive_feed_in < 0.0 && emcmotDebug->coord_tp.reverse_run == TC_DIR_FORWARD) {
            ...
            if (modtpSetRunDir(&emcmotDebug->coord_tp, TC_DIR_REVERSE) != TP_ERR_OK) {
            ...
            if (modtpSetRunDir(&emcmotDebug->coord_tp, TC_DIR_FORWARD) != TP_ERR_OK) {
                // Need to decelerate to a stop first
                adaptive_feed_out = 0.0;
            }
        }
	...
	/* calculate pos_fb */
	if (( hmod_get_homing_at_index_search_wait(joint_num) ) &&
	    ( hmod_get_index_enable(joint_num) == 0 )) {
	...
	if ( IS_EXTRA_JOINT(joint_num) && hmod_get_homed(joint_num) ) {
		...
		if (*(emcmot_hal_data->spindle[spindle_num].spindle_orient)) {
			if (*(emcmot_hal_data->spindle[spindle_num].spindle_orient_fault)) {
				...
				*(emcmot_hal_data->spindle[spindle_num].spindle_orient) = 0;
				...
				modtpAbort(&emcmotDebug->coord_tp);
...
static void process_probe_inputs(void)
{
    ...
    if (emcmotStatus->probing) {
        ...
        } else if (GET_MOTION_INPOS_FLAG() && modtpQueueDepth(&emcmotDebug->coord_tp) == 0) {
        ...
        if(!GET_MOTION_INPOS_FLAG() && modtpQueueDepth(&emcmotDebug->coord_tp)) {
            // running an command
            modtpAbort(&emcmotDebug->coord_tp);
        ...
        for(i=0; i<NO_OF_KINS_JOINTS; i++) {
            ...
            // abort any homing
            if(hmod_get_homing(i)) {
                hmod_set_home_abort(i);
                aborted=1;
            }
...
static void set_operating_mode(void)
{
	...
	modtpClear(&emcmotDebug->coord_tp);
	for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
	    /* point to joint data */
	    joint = &joints[joint_num];
		...
		hmod_set_joint_homing(joint_num,0);
		hmod_set_home_idle(joint_num);
	    }
...
static void handle_jjogwheels(void)
{
	...
	if (emcmotStatus->net_feed_scale < 0.0001 ) {
        ...
        if (hmod_get_home_is_synchronized(joint_num)) {
...
static void handle_ajogwheels(void)
{
	...
	if (hmod_get_homing_is_active()   )   { continue; }
...
static void get_pos_cmds(long period)
{
	...
	for (joint_num = 0; joint_num < ALL_JOINTS; joint_num++) {
	    ...
	    /* compute joint velocity limit */
            if (   (emcmotStatus->motion_state != EMCMOT_MOTION_FREE)
               ...
               if (vel_lim < joint->free_tp.max_vel)
                ...
                /* except if homing, when we set free_tp max vel in hmod_do_homing */
            }
		...
		hmod_set_joint_at_home(joint_num,0);
	...
	while (cubicNeedNextPoint(&(joints[0].cubic))) {
	    ...
	    modtpRunCycle(&emcmotDebug->coord_tp, period); // use the symbol

            /* get new commanded traj pos */
            modtpGetPos(&emcmotDebug->coord_tp, &emcmotStatus->carte_pos_cmd);
	...
	SET_MOTION_INPOS_FLAG(0);
	if (modtpIsDone(&emcmotDebug->coord_tp)) {
	    SET_MOTION_INPOS_FLAG(1);
	}
	break;
    ...
    */
	...
	if ((!GET_JOINT_ACTIVE_FLAG(joint)) || (!hmod_get_homed(joint_num))) {
	    continue;
        }
...
static void update_status(void)
{
	...
	joint_status->homing = hmod_get_homing(joint_num);
	joint_status->homed  = hmod_get_homed(joint_num);
    ...
    */
    ...
    emcmotStatus->depth = modtpQueueDepth(&emcmotDebug->coord_tp);
    emcmotStatus->activeDepth = modtpActiveDepth(&emcmotDebug->coord_tp);
    emcmotStatus->id = modtpGetExecId(&emcmotDebug->coord_tp);
    //KLUDGE add an API call for this
    emcmotStatus->reverse_run = emcmotDebug->coord_tp.reverse_run;
    emcmotStatus->tag = modtpGetExecTag(&emcmotDebug->coord_tp);
    emcmotStatus->motionType = modtpGetMotionType(&emcmotDebug->coord_tp);
    emcmotStatus->queueFull = modtcqFull(&emcmotDebug->coord_tp.queue);
    ...
    if (emcmotDebug->stepping && emcmotDebug->idForStep != emcmotStatus->id) {
      modtpPause(&emcmotDebug->coord_tp);
