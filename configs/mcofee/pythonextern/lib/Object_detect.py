        ...
        if (len(validcontours)>0):
            for i in range(0,len(validcontours)):
                ...
                if self.PRINT_IMG_LABELS ==True:
                    ...
                    cv2.putText(img_output,"X: {:0.2f}".format(x),(int(x),int(y) + 15), cv2.FONT_HERSHEY_SIMPLEX , 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                    cv2.putText(img_output,"Y: {:0.2f} ".format(y),(int(x),int(y) + 30), cv2.FONT_HERSHEY_SIMPLEX , 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                    cv2.putText(img_output,"Angle {:0.2f}".format(angle),(int(x),int(y) + 45), cv2.FONT_HERSHEY_SIMPLEX , 0.5, (0, 0, 255), 1, cv2.LINE_AA)
