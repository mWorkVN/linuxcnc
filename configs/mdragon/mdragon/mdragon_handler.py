...
from qtvcp.widgets.mwork_emclogwitget import mwork_emclogwitget
from qtvcp.widgets.mwork_pickandplace import mwork_pickandplace

#import scaraqtvcp as MILL
#from qtvcp.lib.qt_vismach import mill_xyz as MILL
from qtvcp.lib.qt_vismach import mscara as MILL
...
class HandlerClass:
    def __init__(self, halcomp, widgets, paths):
        ...
        self.machine = MILL.Window(5)
        KEYBIND.add_call('Key_F12','on_keycall_F12')
        KEYBIND.add_call('Key_Pause', 'on_keycall_pause')
        self.w.setWindowTitle('PyQt5 Lesson 4')     
        ...
        self.tab_index_code = (0, 1, 2, 3, 0, 0, 2, 0, 0, 0, 0,9,0)
        ...
        self.onoff_list = ["frame_program", "frame_dro", "frame_override", "frame_status"]
        self.auto_list = ["chk_eoffsets", "cmb_gcode_history"]
        self.axis_c_list = ["label_axis_c", "dro_axis_c", "action_zero_c", "axistoolbutton_c",
                            "action_home_c", "widget_jog_angular", "widget_increments_angular",
                            "c_plus_jogbutton", "c_minus_jogbutton"]
...
""" %(os.path.join(paths.IMAGEDIR,'lcnc_swoop.png'))
    ...
    def initialized__(self):
        ...
        #new self.w.btn_spindle_pause.setEnabled(False)
        self.w.btn_dimensions.setChecked(True)
        self.w.page_buttonGroup.buttonClicked.connect(self.main_tab_changed)
        self.w.mpage_buttonGroup.buttonClicked.connect(self.mpage_changed)
        self.w.filemanager_usb.showMediaDir(quiet = True)
        # init mwork widgets
        self.w.machine3D.addWidget(self.machine)
        
        path = os.path.dirname(os.path.abspath(__file__))
        self.PICKANDPLACE = mwork_pickandplace(widgets=self.w , pathIniFolder = path)
        self.w.PickAndPlace.addWidget(self.PICKANDPLACE)
        self.PICKANDPLACE.setObjectName('PICKANDPLACE')
        self.PICKANDPLACE.hal_init()
        from qtvcp.widgets.mwork_chooseActuator import mwork_chooseActuator
        self.CHOOSEACTUATOR = mwork_chooseActuator(widgets=self.w , pathIniFolder = path)
        self.CHOOSEACTUATOR.setObjectName('CHOOSEACTUATOR')
        self.w.actuator.addWidget(self.CHOOSEACTUATOR)
        self.CHOOSEACTUATOR.hal_init()

        self.EMCLOG = mwork_emclogwitget(path = path)
        self.EMCLOG.setObjectName('EMCLOG')
        self.w.teachingpanel.addWidget(self.EMCLOG)
        self.w.stackedWidget_2.setCurrentIndex(0)
        self.w.mpagestack.setCurrentIndex(0)


        from qtvcp.widgets.mwork_opencv import mwork_opencv
        self.MWORKOPENCV = mwork_opencv()
        self.MWORKOPENCV.testemit.connect(self.testconnect)
        self.MWORKOPENCV.setObjectName('MWORKOPENCV')
        self.w.mworkcamera.addWidget(self.MWORKOPENCV)
        self.MWORKOPENCV.hal_init()
        ...
        self.w.camtab.addWidget(self.MWORKCAMVIEW)
        self.MWORKCAMVIEW.hal_init()
        
        #self.w.yyy.addWidget(EMCLOG)
        #self.EMCLOG.hal_init()

    # hide widgets for A axis if not present
        if "C" not in INFO.AVAILABLE_AXES:
            for i in self.axis_c_list:
                self.w[i].hide()
    ...
    def init_pins(self):
        ...
        pin = self.h.newpin("stsKins.scaraKins", hal.HAL_FLOAT, hal.HAL_IN)
        CALL(['halcmd', 'net', ':kinstype-select', 'mdragon.stsKins.scaraKins'])
        pin.value_changed.connect(self.kins_scara_change)
    ...
    def init_widgets(self):
        self.w.main_tab_widget.setCurrentIndex(12)
        ...
        self.w.slider_maxv_ovr.setValue(1000)
        #info= "Check 3001 - "
...
#                    QtCore.Qt.Key_F3,QtCore.Qt.Key_F4,QtCore.Qt.Key_F5):
    ...
    #########################
    def kins_scara_change(self):
        typekins = self.h['stsKins.scaraKins'] 
        if typekins==0:
            self.w.joint_mode_button.setChecked(False) 
            self.w.world_mode_button.setChecked(True) 
        elif typekins==1:
            self.w.joint_mode_button.setChecked(True) 
            self.w.world_mode_button.setChecked(False) 
    ...
    def _homelabel(self,joint,status):
        if joint == "3":
            if status =="home":
                self.w.dro_axis_c._home_status_polish(5, True)
            else:
                self.w.dro_axis_c._home_status_polish(5, False)
            #print("home ",joint , status , self.w.dro_axis_c.getisHomed())
    ...
    def main_tab_changed(self, btn):
        ...
        if STATUS.is_auto_mode() and index != 9 and index != 11 and index != 12 and index != 8:
        ...
        if index == TAB_SETUP:
            #self.w.jogging_frame.hide()  # chuyen qua tab teach
            self.w.jogging_frame.show()
        else:
            self.w.jogging_frame.show()
        if index == TAB_MAIN:
            self.w.stackedWidget_dro.setCurrentIndex(0)
    def mpage_changed(self, btn):
        index = btn.property("index")
        if index is None: return
        self.w.mpagestack.setCurrentIndex(index)
    ...
    def btn_start_clicked(self, obj):
        if self.w.main_tab_widget.currentIndex() != 0 and self.w.main_tab_widget.currentIndex() != 11 and self.w.main_tab_widget.currentIndex() != 12 and self.w.main_tab_widget.currentIndex() != 8:
            return
    ...
    def btn_home_all_clicked(self, obj):
        if self.home_all is False:
            ...
            if (float(YJoint)>= 0):
                info= "Error 3001 - "
                info=info + "X: {} , Y:{}".format(XJoint,YJoint)
                mess = {'NAME':'MESSAGE', 'ID':'_unhome_', 'MESSAGE':'UNHOME ALL', 'MORE':info, 'TYPE':'OKCANCEL'}
                ACTION.CALL_DIALOG(mess)
            else:   
                ACTION.SET_MACHINE_HOMING(-1)
    ...
    def getReturnJoint(self, joint):
        if self.home_all is True:
            if joint is 3:
                return 5
        return joint
        
    def btn_jog_run_clicked(self):
        ACTION.SET_MANUAL_MODE()
        #joint = self.w.sender().property('joint')
        axisnum = self.w.sender().property('joint')
        joint = INFO.get_jnum_from_axisnum(axisnum)
        direct = self.w.sender().property('direct')
        joint = self.getReturnJoint(joint)
        self.kb_jog(1, joint, direct, 1)
		
    def btn_jog_stop_clicked(self):
        ACTION.SET_MANUAL_MODE()
        #joint = self.w.sender().property('joint')
        axisnum = self.w.sender().property('joint')
        joint = INFO.get_jnum_from_axisnum(axisnum)
        direct = self.w.sender().property('direct')
        joint = self.getReturnJoint(joint)
        self.kb_jog(0, joint, direct, 1)

    def btn_view_machine_clicked(self):
        index = self.w.sender().property('index')
        self.w.stackedWidget_2.setCurrentIndex(index)
        if index == 0:
            angleX = self.w.sender().property('angleX')
            angleY = self.w.sender().property('angleY')
            self.machine.setviewX(angleX)
            self.machine.setviewY(2880)
    # tool frame
    ...
    def slider_maxv_changed(self, value):
        maxpc = (float(value) / INFO.MAX_LINEAR_JOG_VEL) * 2
    ...
    def cam_zoom_changed(self, value):
        self.MWORKOPENCV.scale = float(value) / 10

    def cam_dia_changed(self, value):
        self.MWORKOPENCV.diameter = value

    def cam_rot_changed(self, value):
        self.MWORKOPENCV.rotation = float(value) / 10
    ...
    def disable_spindle_pause(self):
        self.h['eoffset_count'] = 0
        self.h['spindle_pause'] = False
        #new if self.w.btn_spindle_pause.isChecked():
        #new     self.w.btn_spindle_pause.setChecked(False)
    ...
    def kb_jog(self, state, joint, direction, fast = False, linear = True):
        ...
        if STATUS.is_auto_mode() or not STATUS.machine_is_on():# mwork add
    ...
    def enable_auto(self, state):
        ...
        if state is True:
            ...
            if self.w.main_tab_widget.currentIndex() != TAB_PROBE:
                ...
                #self.w.btn_main.setChecked(True)
                #self.w.main_tab_widget.setCurrentIndex(TAB_MAIN)
                self.w.stackedWidget.setCurrentIndex(12)
                self.w.stackedWidget_dro.setCurrentIndex(12)
